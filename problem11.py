...
for (di, dj) in directions:
    ...
    for i in range(i1, i2 + 1):
        if dj == -1:
            ...
            j1 = 0
            j2 = sizex - 1 - (length - 1) * dj
        for j in range(j1, j2 + 1):
            p = product(i, j, di, dj, length)
            if p > result:
                result = p

print "The greatest product of %d adjacent numbers in the same direction \
in the %dx%d grid is %d." % (length, sizex, sizey, result)
